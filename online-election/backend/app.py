...
async def login(request: Request, login_data: LoginRequest):
    try:
        # Check users table
        user = supabase.table("users").select("*").eq("voter_id", login_data.voter_id).execute().data
        if not user:
            # Try username if voter_id not found (for admin login)
            user = supabase.table("users").select("*").eq("username", login_data.voter_id).execute().data
        ...
        user = user[0]
        # Verify password
        if verify_password(login_data.password, user['password']) or login_data.password == user['password']:
            # Create access token
            token = create_access_token(data={"sub": user['voter_id'], "role": user['role']})
            
            # Map DB user to Frontend User model
            formatted_user = {
            ...
            return {"status": "success", "user": formatted_user, "access_token": token, "token_type": "bearer"}
        else:
            raise HTTPException(status_code=401, detail="Invalid Credentials")
...
async def get_final_results(is_admin: bool = False):
    try:
        # Optimization: Don't block admin from seeing real-time tally
        if not is_admin:
            settings = supabase.table("settings").select("*").single().execute().data
            if settings and settings.get("is_active"):
                # For commission, we might still want to show reports but maybe not the final winner
                pass 
        
        # Get all parties and their votes
        parties = supabase.table("parties").select("name,votes").execute().data
        
        # We also need to decrypt votes to get real-time results if they aren't synced to parties table
        votes = supabase.table("votes").select("party_name").execute().data
        tally = {p['name']: 0 for p in parties}
        
        for v in votes:
            try:
                name = decrypt_data(v['party_name'])
                if name in tally:
                    tally[name] += 1
                else:
                    # Handle cases where party might have changed name or something
...
# Removed duplicated final-results endpoint
