  ...
  const handleAdminLogin = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/login', {
        ...
        body: JSON.stringify({
          voter_id: adminLoginData.username,
      ...
      if (data.status === 'success' && data.user.role === 'admin') {
        ...
        showAlert(data.detail || 'Invalid Admin Credentials');
      }
    } catch (error) {
      console.error(error);
      showAlert('Server Error. Please ensure backend is running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCommLogin = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/login', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          voter_id: commLoginData.username,
          password: commLoginData.password
        })
      });
      const data = await res.json();
      if (data.status === 'success' && data.user.role === 'commission') {
        setView('commission-dashboard');
        showAlert('Commission Access Granted', 'success');
      } else {
        showAlert(data.detail || 'Invalid Commission Credentials');
      }
    } catch (error) {
      console.error(error);
