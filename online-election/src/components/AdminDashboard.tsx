...
export function AdminDashboard({ db, addParty, registerUser, updateSettings, onLogout, showAlert }: AdminDashboardProps) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const res2 = await fetch('http://localhost:8000/api/commission/final-results?is_admin=true');
      const data2 = await res2.json();
      if (data2.tally) setTallyData(data2.tally);
    } catch (e) {
            ...
            >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      {db.parties.map((p: any, i: number) => (
                        <div key={i} className="p-8 bg-white border-2 border-slate-50 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 flex items-center gap-8 group relative overflow-hidden">
                          <div className="absolute top-0 right-0 w-32 h-32 bg-[#FF9933]/5 rounded-full -mr-16 -mt-16 group-hover:bg-[#FF9933]/10 transition-colors"></div>

                          <div className="w-24 h-24 bg-slate-50 rounded-3xl p-3 shadow-inner group-hover:scale-110 transition-transform duration-500 overflow-hidden flex items-center justify-center border-2 border-white relative z-10">
                            {p.imageUrl ?
                              <img src={p.imageUrl} alt={p.name} className="w-full h-full object-contain" /> :
                              <span className="text-4xl font-black text-[#003366]">{p.name[0]}</span>
                            }
                          </div>

                          <div className="relative z-10 flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="px-3 py-1 bg-[#003366] text-white text-[8px] font-black uppercase rounded-full tracking-widest">Official</span>
                              <h4 className="text-2xl font-black text-[#003366] uppercase tracking-tighter leading-none">{p.name}</h4>
                            </div>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest flex items-center gap-2">
                              <Flag className="w-3 h-3 text-[#FF9933]" /> {p.symbol || "NO SYMBOL"}
                            </p>
                          </div>

                          <div className="text-right relative z-10">
                            <div className="text-3xl font-black text-[#003366]">{p.votes || 0}</div>
                            <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Total Votes</div>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {activeTab === 'reports' && (
                <div className="space-y-10">
                  ...
                  {/* Live Results Tally Section */}
                  {tallyData && (
                    <Card className="bg-white border-none shadow-xl rounded-[2.5rem] p-10 overflow-hidden relative">
                      <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 rounded-full -mr-32 -mt-32 -z-0"></div>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10 relative z-10">Real-Time Vote Count (Admin Access)</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
                        {Object.entries(tallyData).map(([party, votes]: [string, any], idx) => (
                          <div key={party} className="p-6 bg-slate-50/50 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center gap-4 text-center group hover:bg-white hover:shadow-xl transition-all">
                            <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg" style={{ backgroundColor: ['#003366', '#FF9933', '#4CAF50', '#F44336'][idx % 4] }}>
                              {idx + 1}
                            </div>
                            <div className="space-y-1">
                              <h5 className="font-black text-[#003366] uppercase tracking-tighter truncate w-full">{party}</h5>
                              <p className="text-4xl font-black text-[#003366]">{votes}</p>
                              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Mandates Received</p>
                            </div>
                          </div>
                        ))}
                      </div>
