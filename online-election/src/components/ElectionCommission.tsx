...
export function ElectionCommission({ db, onLogout, showAlert }: ElectionCommissionProps) {
    ...
    const isElectionEnded = () => {
        // If it's not active, it's ended or stopped.
        if (!db.electionSettings.isActive) return true;

        // Even if active, check if it's PAST the end time
        if (db.electionSettings.endTime) {
            const now = new Date();
